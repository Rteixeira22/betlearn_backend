...
export class ChallengesController {
    ...
    async createChallenge(req: Request, res: Response) {
        try {
            const { number, name,  short_description, long_description, image } = req.body
    ...
    async updateUserHasChallengesProgressPercentage(req: Request, res: Response) {
        try {
            ...
            if (progress_percentage === 100) {
                ...
                try {
                    await axios.post(`http://localhost:3000/api/challenges/${userId}/${challengeId}/unblock-next`);
                } catch (axiosError) {
